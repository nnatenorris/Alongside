...
export function buildReplayPlayerHtml(
  videoId: string,
  startSeconds: number,
  reactionUrl: string,
): string {
...
<head>
  ...
  <style>
    ...
    #reaction{
      position:absolute;bottom:12px;right:12px;width:30%;aspect-ratio:9/16;
      border-radius:10px;object-fit:cover;border:2px solid #ffffff40;
    }
  </style>
</head>
<body>
  <div id="player"></div>
  <video id="reaction" src="${reactionUrl}" playsinline muted="false"></video>
  <script src="https://www.youtube.com/iframe_api"></script>
  <script>
    var player;
